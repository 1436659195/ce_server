 ...
 * send 敲键(给 cc/codex/opencode/裸 shell 发输入都一样)。handler 跑在 ce 进程内,直接访问
 * ce 已有的终端缓冲(read)与 terminado stdin(send),零 IPC。
 *
 * tid = 终端在 list 里的序号(1..N),管家据此引用——手机侧不再有 tidMap 注入那套。
 */
export interface ToolDeps {
  /** 终端缓冲:list 名字、read 最近 N 行。 */
  buffers: { list: () => string[]; read: (name: string, lines: number) => string }
...
export function makeButlerTools(deps: ToolDeps): SdkMcpToolDefinition<any>[] {
  const nameByTid = (tid: number): string | undefined => deps.buffers.list()[tid - 1]
  ...
  return [
    tool(
      ...
      {},
      async () => {
        const lines = deps.buffers.list().map(
          (n, i) => `#${i + 1} ${n} · 末行: ${(deps.buffers.read(n, 1) || '(空)').slice(0, 80)}`,
        )
    ...
    tool(
      ...
      async (a) => {
        const name = nameByTid(a.tid)
        const body = name ? deps.buffers.read(name, Math.min(a.lines, 200)) : `(tid ${a.tid} 不存在)`
