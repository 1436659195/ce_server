...
/** mock 缓冲:list + read 按预设返回。 */
const mockBuf = (list: string[], read: (n: string) => string): ToolDeps['buffers'] =>
  ({ list: () => list, read: (n: string, _lines: number) => read(n) }) as ToolDeps['buffers']
...
test('list_terminals:返每终端名 + 末行', async () => {
  const tools = makeButlerTools({ buffers: mockBuf(['t1', 't2'], (n) => (n === 't1' ? 'hello' : '')), send: async () => {} })
...
test('read_terminal:读指定 tid 的尾行', async () => {
  const tools = makeButlerTools({ buffers: mockBuf(['t1', 't2'], () => 'a\nb\nc'), send: async () => {} })
...
test('read_terminal:tid 不存在 → 提示', async () => {
  const tools = makeButlerTools({ buffers: mockBuf(['t1'], () => 'x'), send: async () => {} })
...
test('send_terminal:调 send(name, text)', async () => {
  ...
  const tools = makeButlerTools({ buffers: mockBuf(['t1'], () => ''), send: async (n, t) => { sent.push([n, t]) } })
...
test('send_terminal:tid 不存在 → isError 且不调 send', async () => {
  let called = false
  const tools = makeButlerTools({ buffers: mockBuf(['t1'], () => ''), send: async () => { called = true } })
